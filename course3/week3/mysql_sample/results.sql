...
-- select distinct store_id from store where store_id in (select distinct store_id from sale);
...
select name, avg(total/quantity) from product natural join sale group by product_id;  -- from teachers
